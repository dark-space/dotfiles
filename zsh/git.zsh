if which fzf >/dev/null 2>&1; then
    function fzf-git-add() {
        local selected
        selected=($($dotfiles/bin/unbuffer git status -s | fzf --no-sort --reverse --ansi --preview="git diff --color=always {2..}" --preview-window=up:70% -m | awk '{print $2}'))
        if [[ "$selected" =~ \\S ]]; then
            sed -e 's/\s\+/\n/g' -e 's/^/add /' <<< "$selected"
            git add $selected
        fi
    ...
    function fzf-git-status-widget() {
        local selected
        selected=$($dotfiles/bin/unbuffer git status -sb | fzf --no-sort --reverse --ansi --preview="git diff --color=always {2..}" --preview-window=up:70%)
    }
