    ...
    function fzf-history-widget() {
        ...
        while out=$(read_history $history_type | FZF_DEFAULT_OPTS=$fzf_default_opts fzf); do
            ...
            if [ "$key" = "ctrl-a" ]; then
                ...
                fzf_default_opts+="--with-nth=2.. --preview=\"lines {1} $history_all | sed -e 's/^.*//' | cmdpack 'sed -e \"s/^/[44m/\" -e \"s/$/[0m/\"' 'xargs $dotfiles/bin/unbuffer ls --color=always | head'\" --preview-window=up:30% "
