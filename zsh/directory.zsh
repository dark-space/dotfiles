...
if which fzf >/dev/null 2>&1; then
    ...
    function fzf-directory-widget() {
        ...
        query=""
        while out=$(read_directory $directory_type | fzf --query="$query" --print-query --no-sort --ansi -e +m --expect=ctrl-c,ctrl-d,ctrl-s --preview="cat <<< {} | cmdpack 'sed -e \"s/^/[44m/\" -e \"s/$/[0m/\"' 'xargs $dotfiles/bin/unbuffer ls --color=always | head'" --preview-window=up:30%); do
            local key selected
            query=$(lines 1 <<< "$out")
            key=$(lines 2 <<< "$out")
    ...
    function fzf-cd() {
        ...
        directory_type=${DIRECTORY_TYPE:-"all"}
        query=""
        while out=$(read_directory $directory_type | fzf --query="$query" --print-query --no-sort --ansi -e +m --expect=ctrl-d,ctrl-s --preview="cat <<< {} | cmdpack 'sed -e \"s/^/[44m/\" -e \"s/$/[0m/\"' 'xargs $dotfiles/bin/unbuffer ls --color=always | head'" --preview-window=up:30%); do
            local key selected
            query=$(lines 1 <<< "$out")
            key=$(lines 2 <<< "$out")
            selected=$(lines 3: <<< "$out")
